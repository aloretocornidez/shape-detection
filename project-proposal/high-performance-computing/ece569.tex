...
In our case, the thresholding value in our application was hard hard coded into our application where the value chosen based off of experimentation for our application.
...
There are two methods proposed to adress this storage space issue. 
The first way to adress the issue is by down sampling the image to reduce the image size, making sure to de-noise the image in the down-sampling process. 
By down sampling the image, the image may be capabable of fitting on the GPU's shared memory dependent on the application.
Our GPU, has a shared memory size of 64kB.
Downsampling by a factor of 4, reduces the $700 \times 700$ 8-bit image to a total of 122.5kB of data. 
...
By splitting the circles into four quadrants, each thread block loads only a quarter of the image, reducing the amount of memory sapce required for each memory block to 30.625kB. 
...
While this implemention is very direct, it is is heavily bottlenecked by memory accesses. 
To reduce memory accesses conducted by all threads, the algorithm used to populated the histrogram can be heavily modified.
...
This optimization allows a local accumulator to have a greater effect on the execution time because memory contention would increase amoung the thread blocks and histogram. 
...
This change in the algorithm would also increase computation time in proportion to memory transfer, optimizing the kernel for the use of CUDA streaming.
